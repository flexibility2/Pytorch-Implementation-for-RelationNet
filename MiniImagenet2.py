...
class MiniImagenet(Dataset):
    ...
    def create_batch(self, batchsz):                       # idx  from  data[[name1,name2],[]  ]
        """
    ...
    def __getitem__(self, index):
        ...
        for i, path in enumerate(flatten_support_x):
            support_x[i] = self.transform(path)             #  [setze,c,h,w]  第  setze[i] 个
...
if __name__ == '__main__':
    ...
    db = DataLoader(mini,batch_size=10,num_workers=0,shuffle=True)
    for s,ba in enumerate(db):
        support_x, support_y, query_x, query_y = ba[0],ba[1],ba[2],ba[3]
        print(s,support_x.size())
    print("------------------------")
